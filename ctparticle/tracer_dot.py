    ...
    while ligne != "":
        ...
        if numero_ligne == 1 and ligne_entiers != None:
            if len(ligne_entiers) < 3:
                ...
                print "Forme: `0   x   y`"
                sys.exit(1)
            noeud_depot = noeud_depot + ligne_entiers
        ...
        elif ligne_entiers != None:
            if len(ligne_entiers) < 4:
                print "definir_noeuds_depuis_fichier_noeuds(): erreur ligne %d" % numero_ligne           
                print "Cette ligne correspond à une définition de noeud."
                print "Forme: `num_sommet  x   y  qté`"
                sys.exit(1)
            if ligne_entiers[0] < debut_noeuds_atteignables:
                noeuds_a_couvrir = noeuds_a_couvrir + [ligne_entiers[:4]]
            else:
                noeuds_atteignables = noeuds_atteignables + [ligne_entiers[:4]]
        numero_ligne += 1
        ligne = fichier.readline()        
    return [noeud_depot, noeuds_a_couvrir, noeuds_atteignables]
...
[noeud_depot, noeuds_a_couvrir, noeuds_atteignables] =  definir_noeuds_depuis_fichier_noeuds(sys.argv[1])
